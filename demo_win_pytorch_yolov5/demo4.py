...
import os
os.getcwd()
os.add_dll_directory(r'C:\Users\Limbo\Desktop\code\2024.3\AI\lab3_2\demo')
import sys
sys.path.append(r'C:\Users\Limbo\Desktop\code\2024.3\AI\lab3_2\demo')
import DXGI
import torch
...
def get_data(yes, k, f, c,
    ...
    model = torch.hub.load(r'C:\Users\Limbo\Desktop\code\2024.3\AI\lab3_3\yolov5-master', 'custom', path=r'best.pt', source='local')    
    model.conf=0.65
    g = DXGI.capture(*bbox)
    #m=mss.mss()
    cnt = 0
    ...
    while(1):
        ...
        for t in range(50):
            ...
            results = model(im, augment=False)
            ...
            if d2<15000:
                ...
                Logitech.mouse.move(int(0.7*dx), int(0.7*dy))
                ...
                Logitech.mouse.move(int(1.2*dx), int(1.2*dy))
        ...
        if yes.value == 0:
            ...
            print("每帧延时(即用时)%fms ,YOLO推理平均耗时%fms" % ((t2 - t1)*1000/cnt,(t2 - t1)*1000/cnt-7))
